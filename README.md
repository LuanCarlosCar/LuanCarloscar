<h1 align="center">Olá, eu sou o Luan Carlos 👋</h1>

<p align="center">
  Desenvolvedor Fullstack construindo produtos SaaS web e mobile.<br>
  Hoje no setor de construtech, em transição de sênior para tech lead.
</p>

---

### 🚀 Sobre mim

- 🔭 Trabalho de ponta a ponta: portal web (React, Next.js), apps mobile (React Native, Expo) e backend (Node.js, AdonisJS, MySQL)
- 📱 Foco atual em apps mobile offline-first com sincronização SQLite ↔ MySQL
- 🧠 Explorando IA aplicada ao desenvolvimento: agentic coding, automação de QA e Spec-Driven Development
- 👥 Liderança técnica: arquitetura, code review e mentoria de time
- 🌱 Sempre testando tecnologia nova antes do hype virar consenso

---

### 🛠️ Stack

<div>
  <img align="center" alt="TypeScript" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-plain.svg">
  <img align="center" alt="JavaScript" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-plain.svg">
  <img align="center" alt="React" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg">
  <img align="center" alt="Next.js" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nextjs/nextjs-original.svg">
  <img align="center" alt="Node.js" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original.svg">
  <img align="center" alt="MySQL" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg">
  <img align="center" alt="SQLite" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/sqlite/sqlite-original.svg">
  <img align="center" alt="Docker" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg">
  <img align="center" alt="HTML5" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg">
  <img align="center" alt="CSS3" height="40" width="48" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg">
</div>

<p>
  <em>Também trabalho com:</em> React Native · Expo · Git · REST APIs
</p>

---

### 🔗 Contato

<div align="center">
  <a href="https://www.linkedin.com/in/luan-carlos-a3a08a123/" target="_blank">
    <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white">
  </a>
</div>

---

